...
% default filepath
handles.filepath = '\\zserver2\Data\EyeCamera\';
handles.filename = '';
...
if handles.doPlotting
    ...
    if get(handles.filteredEllipseCheckbox, 'Value')
        ...
        title('');
        set(get(hh, 'Children'), 'LineWidth', 0.5, 'LineStyle', ':', 'EdgeColor', 'g')
    end
    ...
    if get(handles.originalEllipseCheckbox, 'Value')
        ...
        hh = ezplot(handles.originalAxis, str, [handles.state.roi(1) handles.state.roi(3) handles.state.roi(2) handles.state.roi(4)]);
        title('');
        set(get(hh, 'Children'), 'LineWidth', 0.5, 'LineStyle', ':', 'EdgeColor', 'g')
        %     axis off;
    end
...
if get(handles.originalEllipseCheckbox, 'Value')
    hh = ezplot(handles.originalAxis, res.equation{iFrame}, [res.roi(iFrame, 1) res.roi(iFrame, 3) res.roi(iFrame, 2) res.roi(iFrame, 4)]);
    title('');
    set(get(hh, 'Children'), 'LineWidth', 0.5, 'LineStyle', ':', 'EdgeColor', 'g')
    %     axis off;
end
