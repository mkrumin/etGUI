...
% Last Modified by GUIDE v2.5 27-Oct-2014 21:39:09
...
[FileName, PathName] = uigetfile({'*.mj2;*.mp4', 'Our cool et video files'}, 'Select video Files to analyse', hh.filepath, 'MultiSelect', 'on');
if iscell(FileName)
...
hh = guidata(handles.etGUI);
folder_name = uigetdir(hh.filepath);
...
if ~get(hObject, 'Value')
    ...
    if ~isequal(handles.state, 'idle')
        ...
        set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {[datestr(clock, '[HH:MM:SS]'), '   Abort requested...']}))
        if isequal(handles.state, 'blinkDetection')
            set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {'                   Be patient, will stop after the current analysis step is complete...'}))
            set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {'                   Blink detection cannot be interrupted'}))
        elseif isequal(handles.state, 'pupilTracking')
            set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {'                   Be patient, will stop after the current analysis step is complete...'}))
            set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {'                   Pupil tracking can be interrupted from the main GUI'}))
        end
        set(handles.logListbox, 'Value', length(get(handles.logListbox, 'String')));
...
set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {sprintf('%s   Started batch processing...', datestr(clock, '[HH:MM:SS]'))}))
set(handles.logListbox, 'Value', length(get(handles.logListbox, 'String')));

handles.state = 'blinkDetection';
guidata(hObject, handles);

set(hObject, 'String', 'Stop');
set(hObject, 'TooltipString', 'Be patient after clicking, will stop after the current analysis step is complete');
...
for iFile = 1:length(fileList)
    ...
    set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {[datestr(clock, '[HH:MM:SS]'), '   Opening  ', filename, ext, '...']}))
    set(handles.logListbox, 'Value', length(get(handles.logListbox, 'String')));
    eventdata.overwrite = true;
    eventdata.file = fileList{iFile};
    etGUI('openfile_pushtool_ClickedCallback', hh.openfile_pushtool, eventdata, guidata(handles.etGUI));
    disableAll(handles.etGUI);
    set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {[datestr(clock, '[HH:MM:SS]'), '   Blink detection...']}))
    set(handles.logListbox, 'Value', length(get(handles.logListbox, 'String')));
    ...
    guidata(hObject, handles);
    set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {[datestr(clock, '[HH:MM:SS]'), '   Tracking the pupil ...']}))
    set(handles.logListbox, 'Value', length(get(handles.logListbox, 'String')));
    set(hh.analyzeTogglebutton, 'Value', 1);
    set(hh.blinkplotPushbutton, 'Enable', 'on');
    etGUI('analyzeTogglebutton_Callback', hh.analyzeTogglebutton, [], guidata(handles.etGUI));
    disableAll(handles.etGUI);
    set(handles.logListbox, 'String', cat(1, get(handles.logListbox, 'String'), {[datestr(clock, '[HH:MM:SS]'), '   Saving results ...']}))
    set(handles.logListbox, 'Value', length(get(handles.logListbox, 'String')));
    etGUI('save_uipushtool_ClickedCallback', hh.save_uipushtool, [], guidata(handles.etGUI));
end
